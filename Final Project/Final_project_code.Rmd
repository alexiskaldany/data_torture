...
#Kaiyuan's section
#SMART QUESTION:What is the probability that a large, female, vaccinated dog would get adopted quickly (adoption speed of 1 or 2). 
```{r}
logit <- glm(data$AdoptionSpeed ~ data$Type + data$MaturitySize+ data$Vaccinated + data$Gender, data = data, family = "binomial")
```
```{r}
xkabledply(logit, title = paste("Logistic Regression :", format(formula(logit)) ))
```
```{r}
pdt<-data.frame(Type=1,MaturitySize=3,Gender=2,Vaccinated=1)
predict_female <- predict(logit,pdt,typle="response")
predict_female
```
...
```{r}
dataTGMFV = dat[, c('Age','Type','Gender','MaturitySize','FurLength','Vaccinated','AdoptionSpeed')]
dataTGMFV$AdoptionSpeed[dat$AdoptionSpeed == 0|dat$AdoptionSpeed == 1|dat$AdoptionSpeed == 2] = 0
dataTGMFV$AdoptionSpeed[dat$AdoptionSpeed == 3|dat$AdoptionSpeed == 4 ] = 1
...
petFeaturetableAll = glm(AdoptionSpeed ~Type+Gender+Vaccinated, data = dataTGMFV, family = "binomial")
petFeaturetableR2 = glm(AdoptionSpeed ~ Age+Type+Gender+MaturitySize+FurLength+Vaccinated, data = dataTGMFV, family = "binomial")
...
# summary(multinom(AdoptionSpeed ~ Age+Type+Gender+MaturitySize+FurLength+Vaccinated, data=VA))

# dataAov = glm(AdoptionSpeed ~ Type+Gender+MaturitySize+FurLength+Vaccinated , data = dataTGMFV)
# summary(dataAov)
```
```{r}
featureClean = dat[ , c('Age','Type','Gender','MaturitySize','FurLength','Vaccinated')]
...
```{r}
loadPkg("bestglm")
res.bestglm <- bestglm(Xy = dataTGMFV, family = binomial,
            IC = "AIC",                 # Information criteria for
            method = "exhaustive")
summary(res.bestglm)
res.bestglm$BestModels
...
```{r}
tpred <- knn(train = train, test = test, cl=typeknn.trainLabel, k=8)
cross <- CrossTable(typeknn.testLabel, tpred, prop.chisq = FALSE)
cm = confusionMatrix(tpred, reference = as.factor(typeknn.testLabel)) # from caret library
cmaccu = cm$overall['Accuracy']
...
The shape is decent, but the score of `format(auc(h))` is below the recommended 0.8 thresh-hold, but indicates a better fit than the Hosmer-Lemeshow test would indicate. 
...
```{r}
loadPkg("caret") 
cm_puppy_tree = confusionMatrix( predict(puppy_tree, type = "class"), reference = datage$y )
print('Overall: ')
...
text(puppy_tree, use.n=TRUE, all=TRUE, cex=.8) # Kind of a useless graph!!!
```
