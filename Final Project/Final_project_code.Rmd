...
```{r}
featureClean = dat[ , c('Type','Gender','MaturitySize','FurLength','Vaccinated')]
...
scaledPet <- as.data.frame(scale(knnDataTGMFV[1:5], center = TRUE, scale = TRUE))
...
pet_training <- scaledPet[pet_sample==1, 1:5]
pet_test <- scaledPet[pet_sample==2, 1:5]
```
```{r}
#Now we need to create our 'Y' variables or labels need to input into the KNN function
pet.trainLabels <- knnDataTGMFV[pet_sample==1, 6]
pet.testLabels <- knnDataTGMFV[pet_sample==2, 6]
```
