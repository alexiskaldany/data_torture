...
```{r}
mylogit <- glm(new_dat$AdoptionSpeed ~ new_dat$Type + new_dat$MaturitySize+ new_dat$Vaccinated + new_dat$Gender, data = new_dat, family = binomial(link = "logit"))
summary(mylogit)
...
```{r}
prdt<-data.frame(Type=factor(2),Gender=factor(1),MaturitySize=factor(3),
                 Vaccinated=factor(1))
prdt
X <- predict(mylogit,newdata=prdt,typle="response")
X
