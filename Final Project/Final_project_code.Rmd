...
Lets try a KNN model next. First the variables need to be returned to integers to pass into the model and the numerical features need to be scaled.
...
type_pred <- knn(train = train, test = test, cl=typeknn.trainLabel, k=7)

typepredcross <- CrossTable(typeknn.testLabel, type_pred, prop.chisq = FALSE)
```

Okay still not very good. Let's try and remove some features.
...
type_pred2 <- knn(train = train2, test = test2, cl=typeknn2.trainLabel, k=7)

typepredcross2 <- CrossTable(typeknn2.testLabel, type_pred2, prop.chisq = FALSE)
```

Much better! Let's try to change K
