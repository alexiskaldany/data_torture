...
```{r}
levels(new_dat$AdoptionSpeed)[levels(new_dat$AdoptionSpeed)==0] <-1
summary(new_dat)
```
...
```{r}
logit11 <- glm(new_dat$AdoptionSpeed ~ new_dat$Type + new_dat$MaturitySize+ new_dat$Vaccinated + new_dat$Gender, data = new_dat, family = binomial(link = "logit"))
summary(logit11)
```
```{r}
...
```{r}
exp1<-exp(coef(logit))
xkabledply( as.table(exp1), title = "Exponential of coefficients in Logit Reg" )
```

```{r}
pdt<-data.frame(Type=factor(2),Gender=factor(1),MaturitySize=factor(3),Vaccinated=factor(1))
predict_female <- predict(logit11,pdt,typle="response")
...
```{r}
loadPkg("pROC")
prob=predict(logit11, type = "response" )
new_dat$AdoptionSpeed$prob=prob
h <- roc(new_dat$AdoptionSpeed~prob, data=new_dat)
