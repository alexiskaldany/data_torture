...
In this report we decided to ask 4 different questions to further our understanding of animal adoption profiles. First we will go through our preprocessing steps followed by a quick refresher of our EDA from our previous report. Next we will test our ability to predict adoption speed by looking at the probability that a large, female, vaccinated dog would get adopted quickly. Following this we'll look closer at our model of adoption speed prediction and determine if the profile significantly influences adoption speed, and what is the best model would be to analyse it. Next we'll switch our focus and ask if the type of animal (Dog or Cat) can be categorized based off the adoption profile in order to determine if there are underlying patterns to the profiles that we had not discovered. Finally, we'll ask if the young animals (puppies and kittens) can be identified based on their profile, again to determine if there are any patterns or differences in these profiles. 

The goal of our report is to shed light on animal adoption profiles and the speed at which animals get adopted. Our hope is that any pattern in the profiles could allow shelters to correct the profiles they create to increase the chance of a quick adoptions, or potentially lead to proper community interventions to protect the animals before they need a profile in the first place. 

The outline of our report is as follows:\

1. Preprocessing\
2. Background EDA\
3. What is the probability that a large, female, vaccinated dog would get adopted quickly?\
4. Does animal profile influence the adoption speed significantly, and what is the best model (Logistic Regression, Knn) when considering these variables?\
5. Can the type of animal be classified from the adoption profile?\
6. Can puppies/kittens be identified based on their adoption profile?\
7. Conclusions\
8. Bibliography

# Preprocessing

We're using the same dataset as from the midterm assignment. The data comes from Kaggle (https://www.kaggle.com/c/petfinder-adoption-prediction/data) and includes almost 15,000 observations of 23 variables. Each observation is an adoption profile on PetFinder in Malaysia. We chose to keep `r length(dat)` features, as well as discarding any profile that contained more than one animal, and any animal that did not get adopted in the first 100 days. At the end of these cleaning measures we end up with `r nrow(dat)` observations of the `r length(dat)` features. Initially we transformed the categorical dependent variable (adoption speed) into a continuous numerical variable. For the sake of this project we are keeping it as a categorical variable for some of our analyses.
...
The next preprocessing step was to split the data into a train - test split in preparation for modeling. We split the data in a 70/30 split, with the final train set containing `r nrow(train)` observations and the test set containing `r nrow(test)` observations.
...
LogitHoslem
```
The result of GOF test is p-value <2e-16, which indicates that this model may not be good fit based on the given predictor and dependent vairable.So, the last step is the run the ROC curve to see if the roc score is greater than 0.8. If the score is greater than 0.8 it might be a good fit.
...
```
From the ROC Score `r format(score)` is < 0.8, which proves the prediction from this model may not be a accurate prediction. So in the future, we probably will use different combination of variables or different models to improve accuracy and prediction.


#### predicted probability
This is our last step, to predict when a large, female, vaccinated dog would get adopted quickly (adoption speed of 0 and 1). The first step is to create a data frame of these specific conditions, then put it into the model to see the prediction.
```{r}
...
```
### Conclusion
From the predicted probability, we can see that the predicted probability is 0.676, which means the probability of large, female, vaccinated dog would get adopted quickly will get adopted within one week is 67.6%. Since this is not an accurate model, we will seek for other solutions to improve the overall accuracy to make a precise prediction.
...
#summary(reg.best10)
```
These are the results of feature selection. From the results we can see Adjusted R^2, BIC, and Cp do not select photoAmt, and R^2 select all of them.\
...
unloadPkg("leaps")
```
The result of bestglm in AIC is remove VideoAmt. As a result, we put all the variables into the logistic regression first to see whether they are all statistical significant.\
...
cm = confusion_matrix(petFeaturetableAll)
xkabledply( cm, title = "Confusion matrix from Logit Model" )
pred_ = predict(petFeaturetableAll, dataTGMFV[1:8], type="response")
# calculate Accuracy, Precision, Recall rate, Specificity, F1 score

accuracy = (cm[1,1] + cm[2,2])/cm[3,3]
precision = cm[2,2]/cm[3,2]
recallRate = cm[2,2]/cm[2,3]
specificity = cm[1,2]/cm[1,3]
flScore = (2*precision*recallRate) / (precision+recallRate)
...
Recall rate = `r recallRate`.\
Specificity = `r specificity`.\
F1 score = `r flScore`.\
We can see the Accuarcy is `r accuracy`, which means this is not a bad model.\
...
unloadPkg("ResourceSelection")
```
The p-value of `r petLogitHoslem$p.value` is relatively small. Small p-values mean that the model is a poor fit. And the next evaluation is AUC/ROC.\
...
unloadPkg("pROC")
```
The result of AUC/ROC show that this is not a very good model Area under the curve: `r aucRocOfTGMFV$auc[1]`(less than 0.8). After looking at AUC/ROC, let us try McFadden.\
...
petLogitpr2
```
With the McFadden value of `r petLogitpr2['McFadden']`, which is analogous to the coefficient of determination $R^2$, only about `r format(petLogitpr2['McFadden'], digits = 2)` of the variations in y is explained by the explanatory variables in the model. 
...
```{r results='markup'}
ResultDf = ResultDf[ResultDf$k%%2 == 1,]
xkabledply(ResultDf)
bestK = ResultDf[which.max(ResultDf$Total.Accuracy),"k"]
...
```
The best KNN is `r ResultDf[which.max(ResultDf$Total.Accuracy),"k"]`, so let us see the confusion matrix.\


```{r}
pet_pred <- knn(train = pet_training, test = pet_test, cl=pet.trainLabels, k=bestK)
petPREDCross <- CrossTable(pet.testLabels, pet_pred, prop.chisq = FALSE)
cm = caret::confusionMatrix(pet_pred, reference = as.factor(pet.testLabels)) # from caret library
cmaccu = cm$overall['Accuracy']
```
Accuracy = `r format(cm$overall['Accuracy'])`.\
Precision = `r format(cm$byClass['Precision'])`.\
Recall rate =  `r format(cm$byClass['Recall'])`.\
Specificity =  `r format(cm$byClass['Specificity'])`.\
F1 score = `r format(cm$byClass['F1'])`.\
Accuracy = `r format(cm$overall['Accuracy'])`,so this model also looks not good enough.\
...
Because this is a binary classification problem we decided to use a logistic regression model and a K nearest neighbors model. 

## Logistic Regression

Beginning with the logistic model, we ran feature selection to determine what features we should include.
...
The features that were selected using the `AIC` criteria and the exhaustive method are every feature except for `VideoAmt`.

### Model Results

We can now make that model and test it against the animal type. The results of this model are as follows.
...
With a p_value of `r format(typeHoslem$p.value)` we can reject the null hypothesis that this model was a good fit for our dependent variable of animal type. 

### ROC Plot

In order to confirm this finding we wanted to look at the Receiver Operator Curve of this model. This plot shows the true positive rate against the false positive rate. By plotting the model in this way we can see how well above chance the model preforms.
...
The K value that was determined to be the best for this model was k = `r ResultDf[which.max(ResultDf$Total.Accuracy),]$k` with a total accuracy of `r ResultDf[which.max(ResultDf$Total.Accuracy),]$Total.Accuracy`.
...
The K value that was determined to be the best for this model was k = `r ResultDf2[which.max(ResultDf2$Total.Accuracy),]$k` with a total accuracy of `r ResultDf2[which.max(ResultDf2$Total.Accuracy),]$Total.Accuracy`. This accuracy is very slightly better than the previous model, so we will continue with this model going forward.
...
```{r, results = 'markup'}
kval = ResultDf2[which.max(ResultDf2$Total.Accuracy),]$k
tpred <- knn(train = knn_train2, test = knn_test2, cl=trainLabel, k=kval)
cross <- CrossTable(testLabel, tpred, prop.chisq = FALSE)
cm = caret::confusionMatrix(tpred, reference = as.factor(testLabel)) # from caret library
cmaccu = cm$overall['Accuracy']
...
A common problem at animal shelters is difficulty in accurately determining the age of the pet. Many families/prospective pet adopters have specific preferences for younger or older animals. Determining if the other data can generate an accurate prediction of whether or not an animal is very young or not seem like a reasonable first step in determining if age can at all be predicted using this data. As `r puppy_proportion` percent of the pets are young, if there is any hope is predicting age beginning with a relatively balanced output is valuable. 
...
```{r,echo = FALSE}
pup_logit <- glm(y ~ ., data = datage, family = "binomial")
```

```{r, echo=FALSE, results='markup'}
xkabledply(pup_logit, title = paste("Logistic Regression : Puppy ~ All variables"))
```
...
cm_pup = confusion_matrix(pup_logit)
acc = (cm_pup[1,1] + cm_pup[2,2])/c[3,3]
prec = cm_pup[1,1]/(cm_pup[1,1]+cm_pup[2,1])
recall = cm_pup[1,1]/(cm_pup[1,1] + cm_pup[1,2])
F1 = (2*(prec * recall)) / (prec + recall)
```
Accuracy: `r format(acc)`\
Precision: `r format(prec)`\
...
```{r echo = FALSE}
loadPkg("ResourceSelection") 
ageHoslem = hoslem.test(datage$y, fitted(pup_logit))
unloadPkg("ResourceSelection")
```
#### Hosmer and Lemeshow goodness of fit (GOF) test
**H0** = Model predictions are not statistically different to actual values.
**H1** = Model predictions are statistically different than actual values.

-The p-value of the test is `r format(typeHoslem$p.value)`, which is very low. The null hypothesis is rejected, and the alternative hypothesis accepted. According to this test the generated model is not a good fit. Based on this finding, as well as the values generated from the confusion matrix, confidence in the model is declining. 
...
The shape is decent, closer to a parable, doesn't exactly follow the diagonal. However, an AUC score of `r format(auctype)` is below the recommended 0.8 thresh-hold, but indicates a better fit than the Hosmer-Lemeshow test would suggest. 

The McFadden score of `r format(round(typePR['McFadden'] * 100), 0)`% is pretty low. As the McFadden score is essentially a pseudo $R^2$ value for logistic models, only `r format(round(typePR['McFadden'] * 100), 0)`% of the variation in the data is explained by the model.

With an accuracy score of `r format(acc)`, a GOF test indicating the models do not fit together, an AUC `r format(auctype)` < 0.8, and a fairly low pseudo $R^2$ value, it is safe to conclude that the dataset can only very weakly predict the age of an animal using a logistic model.
...
```
Accuracy = `r round(cm_puppy_tree$overall["Accuracy"]*100, digits=2)`%\
Precision = `r round(cm_puppy_tree$byClass['Precision']*100, digits=2)`%\
Recall = `r round(cm_puppy_tree$byClass['Recall']*100, digits=2)`%\
F1 = `r round(cm_puppy_tree$overall["F1"]*100, digits=2)`%\
...
## Conclusion for SMART Question 4:
