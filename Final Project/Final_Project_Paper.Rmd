...
#summary(reg.best10)
```
These are the results of feature selection. From the result we can see Adjusted R^2, BIC, and Cp do not select photoAmt, and R^2 select all of them.\
...
unloadPkg("leaps")
```
The result of bestglm is remove VideoAmt. As a result, we put all the variables into the logistic regression first to see whether they are all statistical significant.\
...
recallRate = cm[1,1]/(cm[1,1] + cm[1,2])
specificity = (cm[1,1]+cm[1,2])/cm[1,3]
