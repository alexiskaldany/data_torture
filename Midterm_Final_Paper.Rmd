...
The p_value of the t-test was less than our alpha of 0.05 (p = `r dc_t$p.value`), allowing us to reject our null hypothesis that there is no difference between the means of the two groups. Because of this we can conclude that there is a difference in adoption speed between cats and dogs.
...
We found that this was also significant (p = `r mf_t$p.value`). So we can reject the null hypothesis that the mean adoption speed between male and female animals is the same. 
...
We ended up with a significant p_value for the anova (p = `r anova_size$'Pr(>F)'`), so we ran a TukeyHSD post-hoc test.
...
Our ANOVA did produce a significant result (p = `r anova_fur$'Pr(>F)'[1]`) allowing us to reject the null hypothesis that the adoption speed was the same across fur lengths. Because we got a significant result we again ran a TukeyHSD post-hoc analysis and found that the difference between each of the fur length groups was significant. We're able to conclude that fur length does impact adoption speed.
...
Our ANOVA did produce a significant result (p = `r anova_vac$'Pr(>F)'[1]`). You can see from our post-hoc analysis we  that the difference between the vaccinated and unvaccinated, and the unvaccinated and unspecified group was significantly different. We can conclude that vaccination status did impact the adoption speed of the animal. 
