...
```
Ok, so PhotoAmt and Age are statistically relevant, but just using numerical values doesn't really generate much predictive power, as evidence by the rather low adjusted R-squared value.

Lets add in the remaining categorical variables
```{r include=T}
lm_all <- lm(ASnum ~ ., data=dat)
summary(lm_all)
print(xkablevif(lm_all))
```
