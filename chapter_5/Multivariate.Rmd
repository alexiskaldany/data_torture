...
```{r results=T}
lm_all <- lm(ASnum ~ .-AdoptionSpeed, data=dat)
summary(lm_all)
xkablevif(lm_all)
```
...
```{r}
dat_lm <- subset(dat, select = -c(AdoptionSpeed, VideoAmt))
lm_subset1 <- lm(ASnum ~., data=dat_lm)
summary(lm_subset1)
```
The fact that our Adjusted R-squared value is a lowly 0.0378 is unsetteling. 
